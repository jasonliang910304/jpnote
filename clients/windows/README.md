...
遠端需先安裝 jpnote 0.7.3 以上。client 會驗證 `jpnote.import.v1`、遠端版本、
...
3. 若同 stable key 會更新／合併，逐筆顯示 type、display、stable key 與 core `changes[].text`；Windows client 不自行重算 diff。
4. 仍有 review／conflict 時停止；否則要求輸入大寫 `IMPORT`。
5. 重新確認 Windows 檔案未變，帶 token 執行正式匯入。
6. 只有遠端明確成功且 token 相同後，才詢問是否刪除本機來源檔。
...
預設移除兩種 PowerShell 的精確 0.7.3 目錄，不刪 `_backups` 或其他版本。
